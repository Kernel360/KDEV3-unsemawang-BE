...
@RequiredArgsConstructor
	...
	private final ActiveMemberService activeMemberRedisService;
	...
	public void handleSessionConnect(SessionConnectEvent event) {
		...
		if (userId == null) {
			log.error("인증되지 않은 사용자의 접근입니다, " + event.getUser());
			return;
		}
		...
		ActiveMemberSaveRequest saveRequest = ActiveMemberSaveRequest.builder()
			...
			.build();
		activeMemberRedisService.saveActiveMember(saveRequest);
	...
	public void handleSessionSubscribe(SessionSubscribeEvent event) {
		...
		if (destination != null && destination.startsWith("/topic/active/status/")) {
			...
			// 활동 상태 공유
			ActiveMember activeMember = activeMemberRedisService.findActiveMemberById(memberId);
