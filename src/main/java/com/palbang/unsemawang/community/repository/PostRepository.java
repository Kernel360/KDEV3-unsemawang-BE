...
public interface PostRepository extends JpaRepository<Post, Long>, PostRepositoryCustom {
    ...
    AND (
        (:searchType = 'all' AND (
            ...
            (p.isAnonymous = false AND LOWER(p.member.nickname) LIKE LOWER(CONCAT('%', :keyword, '%')))
        ))
        ...
        OR (:searchType = 'writer' AND p.isAnonymous = false AND LOWER(p.member.nickname) LIKE LOWER(CONCAT('%', :keyword, '%')))
    )
    AND p.isVisible = true
    AND p.isDeleted = false
    ORDER BY p.id DESC
